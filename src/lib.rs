...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::Elapsed => write!(f, "Failed to get elapsed tiem since last datapoint"),
            Error::Calculation => write!(f, "Calculation error"),
        }
    }
}
...
impl Meter {
    ...
    pub fn snapshot(&mut self) -> Result<DataPoint, Error> {
        let duration = self
            .last_data_point
            .datetime
            .elapsed()
...
impl House {
    ...
    fn consume(&mut self, duration: Duration) -> Result<f32, Error> {
        ...
        let household_consumption = (self.occupants.checked_add(1).ok_or(Error::Calculation)?)
            as f32
            * IDLE_CONSUMPTION
            * hours;
